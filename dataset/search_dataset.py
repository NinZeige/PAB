...
class TextMaskingGenerator:
    ...
    def __call__(self, text_ids):  # tokens: [CLS] + ...
        ...
        for pos in cand_pos:
            ...
            if (
                (self.skipgram_prb > 0)
                and (self.skipgram_size >= 2)
                and (rand() < self.skipgram_prb)
            ):
                # ngram
                cur_skipgram_size = randint(2, self.skipgram_size)
                if self.mask_whole_word:
                    st_pos, end_pos = _expand_whole_word(pos, pos + cur_skipgram_size)
                else:
